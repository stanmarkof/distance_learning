...
namespace distant.Controllers
{
    ...
    public class LecturerController : Controller
    {
        ...
        public async Task<IActionResult> OpenTest(int id)
        {
            ...
            // Устанавливаем состояние теста как "открытый"
            test.IsOpen = true;
            await _context.SaveChangesAsync();

            return RedirectToAction(nameof(TestPage), new { id = test.Id });
        }

        // Закрыть тест
        [HttpPost]
        public async Task<IActionResult> CloseTest(int id)
        {
            var test = await _context.Tests.FindAsync(id);
            if (test == null)
            {
            ...
            // Устанавливаем состояние теста как "закрытый"
            test.IsOpen = false;
            await _context.SaveChangesAsync();
